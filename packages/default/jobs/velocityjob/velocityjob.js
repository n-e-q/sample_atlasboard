...
module.exports = {
  ...
  onRun: function (config, dependencies, jobCallback) {
    ...
    /*
     ...
     It is a good idea to cover this with unit tests as well (see test/velocityjob file)
     ...
     Have a look at test/velocityjob for an example of how to unit tests this easily by mocking easyRequest calls
    ...
    dependencies.easyRequest.HTML(config.fullRequest, function (err, html) {
    	if(!err){
    		...
    		var numIters = projArray.length;
    		var velocities = [];
    		
			var totalAcceptedPoints = 0;
    		for(var i = 0; i < numIters; i++){
    			var iterPointer = projArray[i];
    			var numStories = iterPointer.stories.length;
    			for(var j = 0; j < numStories; j++){
    				if(iterPointer.stories[j].current_state == "accepted" && iterPointer.stories[j].story_type == "feature")
    					totalAcceptedPoints += iterPointer.stories[j].estimate;
    			}
    			velocities.push(Math.floor(totalAcceptedPoints / (i+1)));
    		}
